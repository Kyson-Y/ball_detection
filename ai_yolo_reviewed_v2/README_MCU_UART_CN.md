...
- 788 个有效位置样本，5 个短暂 `no_candidate`
- 有效位置最大相邻变化 `23.541 mm`
- 最大推算速度 `689.1 mm/s`
- 超过 `30 mm` 或 `1000 mm/s` 的异常跳变 `0` 次
- UART 写入错误 `0` 次

以上 UART 结果是 MaixCAM 发送端的软件写入状态。MSPM0 仍需按本文最后的验收清单
统计 CRC、序号丢包和接收超时，才能完成物理链路闭环验证。

短暂掉检时，发送包会清除 `DETECTED` 并把位置和速度字段置零。如果接收端
忽略 `DETECTED`，本次测试中会看到最大 `104.266 mm` 的假跳变。因此控制端
必须先判断标志位，再决定是否使用位置。
...
```mermaid
flowchart TD
    A["读取 640x480 相机帧"] --> B["裁剪 ROI: x=0, y=144, 640x192"]
    B --> C["缩放为 320x96 RGB"]
    C --> D["YOLO11 NPU 推理"]
    D --> E["类别、尺寸和宽高比筛选"]
    E --> F["多候选连续性关联"]
...
2. 从完整画面裁剪 `(x=0, y=144, width=640, height=192)` 的水管 ROI。
3. 将 ROI 缩放为模型固定输入 `320x96 RGB`。因此模型横坐标乘以 2 即可映射回
   当前 640 像素 ROI。
...
每个 YOLO 输出还要通过以下几何筛选，单位均为 `320x96` 模型像素：
